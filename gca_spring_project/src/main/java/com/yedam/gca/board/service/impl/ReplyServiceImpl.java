...
public class ReplyServiceImpl implements ReplyService {
	...
	public List<AdReplyVO> list(int ad_num, int start, int end, UserDetails user) {
		AdBoardVO advo =null;
		try {
			advo =boardDAO.read(ad_num);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		
		List<AdReplyVO> items = replyDao.list(ad_num, start, end);
		// 세션에서 현재 사용자 id값 저장
		// String m_id = (String) session.getAttribute("m_id");
		String m_id = (String) user.getUsername();
		for (AdReplyVO vo : items) {
			// 댓글 목록중에 중에 비밀 댓글이 있을 경우
			if (vo.getAdr_hidden().equals("y")) {
				if (m_id == null) { // 비로그인 상태면 비밀 댓글로 처리
					vo.setAdr_content("비밀 댓글입니다.");
				} else { // 로그인 상태일 경우
					String writer = advo.getM_id(); // 게시물 작성자 저장
