	...
	<table class="table">
		...
		<c:forEach var="row" items="${list}">
			<tr>
				<td width="30%">
					<div align="center">
					<img width="35px" height="35px" s src="${pageContext.request.contextPath }/resources/images/Characters/${row.m_image_cd}.gif">
					<br><p align="center">${row.m_nick}</p>
					</div> 
					<%-- (<fmt:formatDate value="${row.adr_regdate}" pattern="yyyy-MM-dd" />) --%>
				</td>
				<td width="40%">${row.adr_content}</td>
				<td style="vertical-align:center;" width="30%">
				<!-- 본인 댓글만 수정버튼 생성되도록 처리 --> 
					<c:if test="${m_id == row.m_id}">
						<button type="button" id="btnModify" style="background-color: #FE9191; color: white;" onclick="showReplyModify('${row.adr_num}')">
							댓글<br>수정
						</button>
					</c:if>
				</td>
...
<%-- 
		...
		<tr style="text-align: center;">
			<td>
				<!-- 현재 페이지 블럭이 1보다 크면 처음페이지로 이동 --> <c:if
				...
				</c:if> <!-- 페이지 블럭 처음부터 마지막 블럭까지 --> <c:forEach var="num"
					...
					test="${replyPager.curBlock <= replyPager.totBlock}">
